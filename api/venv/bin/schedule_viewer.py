#!/home/derek-alves/Documentos/teste/venv/bin/python
...
class ScheduleRequestHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        scheme, host, path, x, params, fragment = urllib.parse.urlparse(
            self.path
        )
        parsed_params = {}
    ...
    def OpenFile(self, filename):
        ...
        if mime_type.startswith("text/"):
            ...
            mode = "rb"
        return (
            open(os.path.join(self.server.file_dir, filename), mode),
            mime_type,
        )
    ...
    def handle_GET_home(self):
        ...
        agency = ", ".join(a.agency_name for a in schedule.GetAgencyList())
    ...
    def handle_json_GET_stoptrips(self, params):
        ...
        for time, (trip, index), tp in time_trips:
            ...
            result.append(
                (time, (trip.trip_id, trip_name, trip.service_id), tp)
            )
        return result
...
def main(RequestHandlerClass=ScheduleRequestHandler):
    ...
    parser.add_option(
        "--port", dest="port", type="int", help="port on which to listen"
    )
    parser.add_option(
    ...
    print(
        "To view, point your browser at http://localhost:%d/"
        % (server.server_port)
    )
